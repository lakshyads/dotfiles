...
All configs in this repo either live under `home/.config/*` (edit-in-place symlinks — see `home.nix`'s `home.file` entries) or are generated by home-manager from `home.nix`'s `programs.zsh` / `programs.starship` blocks.
